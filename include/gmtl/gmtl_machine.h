...
/** This header provides basic platform abstraction layer by hooking up appropriate
    ...
    __GMTL_USE_GENERIC_PART_WORD_CAS
    __GMTL_USE_GENERIC_PART_WORD_FETCH_ADD
    __GMTL_USE_GENERIC_PART_WORD_FETCH_STORE
    __GMTL_USE_GENERIC_FETCH_ADD
    __GMTL_USE_GENERIC_FETCH_STORE
    __GMTL_USE_GENERIC_DWORD_FETCH_ADD
    __GMTL_USE_GENERIC_DWORD_FETCH_STORE
    __GMTL_USE_GENERIC_HALF_FENCED_LOAD_STORE
    __GMTL_USE_GENERIC_FULL_FENCED_LOAD_STORE
    ...
    Additionally __GMTL_64BIT_ATOMICS can be set to 0 on a 32-bit architecture to
    ...
    __GMTL_machine_cmpswp4( volatile void *ptr, int32_t value, int32_t comparand )
        Must be provided if __GMTL_USE_FENCED_ATOMICS is not set.

    __GMTL_machine_cmpswp8( volatile void *ptr, int32_t value, int64_t comparand )
        Must be provided for 64-bit architectures if __GMTL_USE_FENCED_ATOMICS is not set,
        and for 32-bit architectures if __GMTL_64BIT_ATOMICS is set

    __GMTL_machine_<op><S><fence>(...), where
        <op> = {cmpswp, fetchadd, fetchstore}
        <S> = {1, 2, 4, 8}
        <fence> = {full_fence, acquire, release, relaxed}
        Must be provided if __GMTL_USE_FENCED_ATOMICS is set.

    __GMTL_control_consistency_helper()
...
// request default implementation by setting appropriate __GMTL_USE_GENERIC_XXX macro(s).
//
...
#pragma managed(push, off)
        ...
        #if (TBB_USE_GCC_BUILTINS && __GMTL_GCC_BUILTIN_ATOMICS_PRESENT)
...
#pragma managed(pop)
    ...
    #elif __GMTL_GCC_BUILTIN_ATOMICS_PRESENT
        #include "machine/gcc_generic.h"
    #endif
    ...
    #define __GMTL_Yield() sched_yield()

#endif /* OS selection */

#ifndef __GMTL_64BIT_ATOMICS
    #define __GMTL_64BIT_ATOMICS 1
#endif

// Special atomic functions
#if __GMTL_USE_FENCED_ATOMICS
    #define __GMTL_machine_cmpswp1   __GMTL_machine_cmpswp1full_fence
    #define __GMTL_machine_cmpswp2   __GMTL_machine_cmpswp2full_fence
    #define __GMTL_machine_cmpswp4   __GMTL_machine_cmpswp4full_fence
    #define __GMTL_machine_cmpswp8   __GMTL_machine_cmpswp8full_fence

    #if __GMTL_WORDSIZE==8
        #define __GMTL_machine_fetchadd8             __GMTL_machine_fetchadd8full_fence
        #define __GMTL_machine_fetchstore8           __GMTL_machine_fetchstore8full_fence
        #define __GMTL_FetchAndAddWrelease(P,V)      __GMTL_machine_fetchadd8release(P,V)
        #define __GMTL_FetchAndIncrementWacquire(P)  __GMTL_machine_fetchadd8acquire(P,1)
        #define __GMTL_FetchAndDecrementWrelease(P)  __GMTL_machine_fetchadd8release(P,(-1))
    #else
        #error Define macros for 4-byte word, similarly to the above __GMTL_WORDSIZE==8 branch.
    #endif /* __GMTL_WORDSIZE==4 */
#else /* !__GMTL_USE_FENCED_ATOMICS */
    #define __GMTL_FetchAndAddWrelease(P,V)      __GMTL_FetchAndAddW(P,V)
    #define __GMTL_FetchAndIncrementWacquire(P)  __GMTL_FetchAndAddW(P,1)
    #define __GMTL_FetchAndDecrementWrelease(P)  __GMTL_FetchAndAddW(P,(-1))
#endif /* !__GMTL_USE_FENCED_ATOMICS */

#if __GMTL_WORDSIZE==4
    #define __GMTL_CompareAndSwapW(P,V,C)    __GMTL_machine_cmpswp4(P,V,C)
    #define __GMTL_FetchAndAddW(P,V)         __GMTL_machine_fetchadd4(P,V)
    #define __GMTL_FetchAndStoreW(P,V)       __GMTL_machine_fetchstore4(P,V)
#elif  __GMTL_WORDSIZE==8
    #if __GMTL_USE_GENERIC_DWORD_LOAD_STORE || __GMTL_USE_GENERIC_DWORD_FETCH_ADD || __GMTL_USE_GENERIC_DWORD_FETCH_STORE
        #error These macros should only be used on 32-bit platforms.
    #endif

    #define __GMTL_CompareAndSwapW(P,V,C)    __GMTL_machine_cmpswp8(P,V,C)
    #define __GMTL_FetchAndAddW(P,V)         __GMTL_machine_fetchadd8(P,V)
    #define __GMTL_FetchAndStoreW(P,V)       __GMTL_machine_fetchstore8(P,V)
#else /* __GMTL_WORDSIZE != 8 */
    #error Unsupported machine word size.
#endif /* __GMTL_WORDSIZE */

#ifndef __GMTL_Pause
    inline void __GMTL_Pause(int32_t) {
        __GMTL_Yield();
    }
#endif
...
inline void atomic_fence () { __GMTL_full_memory_fence(); }
...
inline T __GMTL_MaskedCompareAndSwap (volatile T *ptr, T value, T comparand ) {
    volatile uint32_t * base = (uint32_t*)( (uintptr_t)ptr & ~(uintptr_t)0x3 );
#if __GMTL_BIG_ENDIAN
    ...
    for(;;) {
        ...
        // __GMTL_CompareAndSwap4 presumed to have full fence.
        // Cast shuts up /Wp64 warning
        result = (uint32_t)__GMTL_machine_cmpswp4( base, new_value, old_value );
...
template<size_t S, typename T>
inline T __GMTL_CompareAndSwapGeneric (volatile void *ptr, T value, T comparand );

template<>
inline uint8_t __GMTL_CompareAndSwapGeneric <1,uint8_t> (volatile void *ptr, uint8_t value, uint8_t comparand ) {
#if __GMTL_USE_GENERIC_PART_WORD_CAS
    return __GMTL_MaskedCompareAndSwap<1,uint8_t>((volatile uint8_t *)ptr,value,comparand);
#else
    return __GMTL_machine_cmpswp1(ptr,value,comparand);
#endif
}

template<>
inline uint16_t __GMTL_CompareAndSwapGeneric <2,uint16_t> (volatile void *ptr, uint16_t value, uint16_t comparand ) {
#if __GMTL_USE_GENERIC_PART_WORD_CAS
    return __GMTL_MaskedCompareAndSwap<2,uint16_t>((volatile uint16_t *)ptr,value,comparand);
#else
    return __GMTL_machine_cmpswp2(ptr,value,comparand);
#endif
}

template<>
inline uint32_t __GMTL_CompareAndSwapGeneric <4,uint32_t> (volatile void *ptr, uint32_t value, uint32_t comparand ) {
    // Cast shuts up /Wp64 warning
    return (uint32_t)__GMTL_machine_cmpswp4(ptr,value,comparand);
}

#if __GMTL_64BIT_ATOMICS
template<>
inline uint64_t __GMTL_CompareAndSwapGeneric <8,uint64_t> (volatile void *ptr, uint64_t value, uint64_t comparand ) {
    return __GMTL_machine_cmpswp8(ptr,value,comparand);
}
#endif

template<size_t S, typename T>
inline T __GMTL_FetchAndAddGeneric (volatile void *ptr, T addend) {
    atomic_backoff b;
    T result;
    for(;;) {
        result = *reinterpret_cast<volatile T *>(ptr);
        // __GMTL_CompareAndSwapGeneric presumed to have full fence.
        if( __GMTL_CompareAndSwapGeneric<S,T> ( ptr, result+addend, result )==result )
            break;
        b.pause();
    }
    return result;
...
template<size_t S, typename T>
inline T __GMTL_FetchAndStoreGeneric (volatile void *ptr, T value) {
    atomic_backoff b;
    T result;
    for(;;) {
        result = *reinterpret_cast<volatile T *>(ptr);
        // __GMTL_CompareAndSwapGeneric presumed to have full fence.
        if( __GMTL_CompareAndSwapGeneric<S,T> ( ptr, value, result )==result )
            break;
        b.pause();
    }
    return result;
...
#if __GMTL_USE_GENERIC_PART_WORD_CAS
#define __GMTL_machine_cmpswp1 tbb::internal::__GMTL_CompareAndSwapGeneric<1,uint8_t>
#define __GMTL_machine_cmpswp2 tbb::internal::__GMTL_CompareAndSwapGeneric<2,uint16_t>
#endif

#if __GMTL_USE_GENERIC_FETCH_ADD || __GMTL_USE_GENERIC_PART_WORD_FETCH_ADD
#define __GMTL_machine_fetchadd1 tbb::internal::__GMTL_FetchAndAddGeneric<1,uint8_t>
...
#if __GMTL_USE_GENERIC_FETCH_ADD || __GMTL_USE_GENERIC_DWORD_FETCH_ADD
#define __GMTL_machine_fetchadd8 tbb::internal::__GMTL_FetchAndAddGeneric<8,uint64_t>
#endif

#if __GMTL_USE_GENERIC_FETCH_STORE || __GMTL_USE_GENERIC_PART_WORD_FETCH_STORE
#define __GMTL_machine_fetchstore1 tbb::internal::__GMTL_FetchAndStoreGeneric<1,uint8_t>
...
#if __GMTL_USE_GENERIC_FETCH_STORE || __GMTL_USE_GENERIC_DWORD_FETCH_STORE
#define __GMTL_machine_fetchstore8 tbb::internal::__GMTL_FetchAndStoreGeneric<8,uint64_t>
#endif

#if __GMTL_USE_FETCHSTORE_AS_FULL_FENCED_STORE
#define __GMTL_MACHINE_DEFINE_ATOMIC_SELECTOR_FETCH_STORE(S)                                             \
    atomic_selector<S>::word atomic_selector<S>::fetch_store ( volatile void* location, word value ) {  \
        return __GMTL_machine_fetchstore##S( location, value );                                          \
    }

__GMTL_MACHINE_DEFINE_ATOMIC_SELECTOR_FETCH_STORE(1)
__GMTL_MACHINE_DEFINE_ATOMIC_SELECTOR_FETCH_STORE(2)
__GMTL_MACHINE_DEFINE_ATOMIC_SELECTOR_FETCH_STORE(4)
__GMTL_MACHINE_DEFINE_ATOMIC_SELECTOR_FETCH_STORE(8)

#undef __GMTL_MACHINE_DEFINE_ATOMIC_SELECTOR_FETCH_STORE
#endif /* __GMTL_USE_FETCHSTORE_AS_FULL_FENCED_STORE */

#if __GMTL_USE_GENERIC_DWORD_LOAD_STORE
inline void __GMTL_machine_store8 (volatile void *ptr, int64_t value) {
    for(;;) {
        int64_t result = *(int64_t *)ptr;
        if( __GMTL_machine_cmpswp8(ptr,value,result)==result ) break;
    }
}

inline int64_t __GMTL_machine_load8 (const volatile void *ptr) {
    ...
    return __GMTL_machine_cmpswp8(const_cast<volatile void *>(ptr),anyvalue,anyvalue);
}
#endif /* __GMTL_USE_GENERIC_DWORD_LOAD_STORE */

#if __GMTL_USE_GENERIC_HALF_FENCED_LOAD_STORE
...
struct machine_load_store {
    static T load_with_acquire ( const volatile T& location ) {
        ...
        __GMTL_acquire_consistency_helper();
        return to_return;
    }
    static void store_with_release ( volatile T &location, T value ) {
        __GMTL_release_consistency_helper();
        location = value;
    }
};

#if __GMTL_WORDSIZE==4 && __GMTL_64BIT_ATOMICS
template <typename T>
struct machine_load_store<T,8> {
    static T load_with_acquire ( const volatile T& location ) {
        return (T)__GMTL_machine_load8( (const volatile void*)&location );
    }
    static void store_with_release ( volatile T& location, T value ) {
        __GMTL_machine_store8( (volatile void*)&location, (int64_t)value );
    }
};
#endif /* __GMTL_WORDSIZE==4 && __GMTL_64BIT_ATOMICS */
#endif /* __GMTL_USE_GENERIC_HALF_FENCED_LOAD_STORE */
...
#else /* !__GMTL_USE_FETCHSTORE_AS_FULL_FENCED_STORE */
    static void store ( volatile T &location, T value ) {
...
#if __GMTL_WORDSIZE==4 && __GMTL_64BIT_ATOMICS
/** The implementation does not use functions __GMTL_machine_load8/store8 as they
...
struct machine_load_store_seq_cst<T,8> {
    static T load ( const volatile T& location ) {
        ...
        return __GMTL_machine_cmpswp8( (volatile void*)const_cast<volatile T*>(&location), anyvalue, anyvalue );
    }
    static void store ( volatile T &location, T value ) {
        int64_t result = (volatile int64_t&)location;
        while ( __GMTL_machine_cmpswp8((volatile void*)&location, (int64_t)value, result) != result )
...
#endif /* __GMTL_WORDSIZE==4 && __GMTL_64BIT_ATOMICS */
#endif /* __GMTL_USE_GENERIC_RELAXED_LOAD_STORE */

template<typename T>
inline T __GMTL_load_with_acquire(const volatile T &location) {
    return machine_load_store<T,sizeof(T)>::load_with_acquire( location );
}
template<typename T, typename V>
inline void __GMTL_store_with_release(volatile T& location, V value) {
    machine_load_store<T,sizeof(T)>::store_with_release( location, T(value) );
}
//! Overload that exists solely to avoid /Wp64 warnings.
inline void __GMTL_store_with_release(volatile size_t& location, size_t value) {
    machine_load_store<size_t,sizeof(size_t)>::store_with_release( location, value );
}

template<typename T>
inline T __GMTL_load_full_fence(const volatile T &location) {
    return machine_load_store_seq_cst<T,sizeof(T)>::load( location );
}
template<typename T, typename V>
inline void __GMTL_store_full_fence(volatile T& location, V value) {
    machine_load_store_seq_cst<T,sizeof(T)>::store( location, T(value) );
}
//! Overload that exists solely to avoid /Wp64 warnings.
inline void __GMTL_store_full_fence(volatile size_t& location, size_t value) {
    machine_load_store_seq_cst<size_t,sizeof(size_t)>::store( location, value );
}

template<typename T>
inline T __GMTL_load_relaxed (const volatile T& location) {
    return machine_load_store_relaxed<T,sizeof(T)>::load( const_cast<T&>(location) );
}
template<typename T, typename V>
inline void __GMTL_store_relaxed ( volatile T& location, V value ) {
    machine_load_store_relaxed<T,sizeof(T)>::store( const_cast<T&>(location), T(value) );
}
//! Overload that exists solely to avoid /Wp64 warnings.
inline void __GMTL_store_relaxed ( volatile size_t& location, size_t value ) {
...
#ifndef __GMTL_TypeWithAlignmentAtLeastAsStrict

#if __GMTL_ATTRIBUTE_ALIGNED_PRESENT

#define __GMTL_DefineTypeWithAlignment(PowerOf2)       \
struct __GMTL_machine_type_with_alignment_##PowerOf2 { \
    uint32_t member[PowerOf2/sizeof(uint32_t)];       \
} __attribute__((aligned(PowerOf2)));
#define __GMTL_alignof(T) __alignof__(T)

#elif __GMTL_DECLSPEC_ALIGN_PRESENT
...
#define __GMTL_alignof(T) __alignof(T)

#else /* A compiler with unknown syntax for data alignment */
#error Must define __GMTL_TypeWithAlignmentAtLeastAsStrict(T)
#endif

/* Now declare types aligned to useful powers of two */
// TODO: Is __GMTL_DefineTypeWithAlignment(8) needed on 32 bit platforms?
__GMTL_DefineTypeWithAlignment(16)
__GMTL_DefineTypeWithAlignment(32)
__GMTL_DefineTypeWithAlignment(64)

typedef __GMTL_machine_type_with_alignment_64 __GMTL_machine_type_with_strictest_alignment;
...
template<> struct type_with_alignment<16> {__GMTL_machine_type_with_alignment_16 member; };
...
struct work_around_alignment_bug {
    static const size_t alignment = __GMTL_alignof(T);
};
#define __GMTL_TypeWithAlignmentAtLeastAsStrict(T) gmtl::internal::type_with_alignment<gmtl::internal::work_around_alignment_bug<sizeof(T),T>::alignment>
#else
#define __GMTL_TypeWithAlignmentAtLeastAsStrict(T) tbb::internal::type_with_alignment<__GMTL_alignof(T)>
#endif  /* __GMTL_ALIGNOF_NOT_INSTANTIATED_TYPES_BROKEN */

#endif  /* __GMTL_TypeWithAlignmentAtLeastAsStrict */
...
using gmtl::internal::__GMTL_store_with_release;

// Mapping historically used names to the ones expected by atomic_load_store_traits
#define __GMTL_load_acquire  __GMTL_load_with_acquire
#define __GMTL_store_release __GMTL_store_with_release

#ifndef __GMTL_Log2
inline intptr_t __GMTL_Log2( uintptr_t x ) {
    if( x==0 ) return -1;
    intptr_t result = 0;
    uintptr_t tmp;
#if __GMTL_WORDSIZE>=8
...
#ifndef __GMTL_AtomicOR
inline void __GMTL_AtomicOR( volatile void *operand, uintptr_t addend ) {
    tbb::internal::atomic_backoff b;
    for(;;) {
        uintptr_t tmp = *(volatile uintptr_t *)operand;
        uintptr_t result = __GMTL_CompareAndSwapW(operand, tmp|addend, tmp);
        if( result==tmp ) break;
        b.pause();
    }
}
...
#ifndef __GMTL_AtomicAND
inline void __GMTL_AtomicAND( volatile void *operand, uintptr_t addend ) {
    tbb::internal::atomic_backoff b;
    for(;;) {
        uintptr_t tmp = *(volatile uintptr_t *)operand;
        uintptr_t result = __GMTL_CompareAndSwapW(operand, tmp&addend, tmp);
...
#ifndef __GMTL_Flag
typedef unsigned char __GMTL_Flag;
#endif
typedef __GMTL_atomic __GMTL_Flag __GMTL_atomic_flag;

#ifndef __GMTL_TryLockByte
inline bool __GMTL_TryLockByte( __GMTL_atomic_flag &flag ) {
    return __GMTL_machine_cmpswp1(&flag,1,0)==0;
}
#endif

#ifndef __GMTL_LockByte
inline __GMTL_Flag __GMTL_LockByte( __GMTL_atomic_flag& flag ) {
    if ( !__GMTL_TryLockByte(flag) ) {
        gmtl::internal::atomic_backoff b;
        do {
            b.pause();
        } while ( !__GMTL_TryLockByte(flag) );
    }
    return 0;
}
#endif

#define __GMTL_UnlockByte __GMTL_store_with_release

#ifndef __GMTL_ReverseByte
inline unsigned char __GMTL_ReverseByte(unsigned char src) {
    return gmtl::internal::reverse<unsigned char>::byte_table[src];
}
#endif

template<typename T>
T __GMTL_ReverseBits(T src) {
    T dst;
    unsigned char *original = (unsigned char *) &src;
    unsigned char *reversed = (unsigned char *) &dst;

    for( int i = sizeof(T)-1; i >= 0; i-- )
        reversed[i] = __GMTL_ReverseByte( original[sizeof(T)-i-1] );
